...
describe('Audio and TTS routes', () => {
  ...
  it('posts synthesis requests to /v1/tts/synthesize with access_mode', async () => {
    mockFetch.mockResolvedValueOnce(
      jsonResponse({
        data: {
          ...
          provider: 'openai',
          chunks: 1,
          speech_profile: 'developer_summary',
          speech_profile_version: '2026-04-29.1',
          prepared_char_count: 18,
    ...
    await sm.tts.synthesize({
      text: 'Hello world',
      speechProfile: 'developer_summary',
      accessMode: 'owner_private'
    });
...
describe('Error Code Mapping', () => {
  ...
  it('should use backend error code when provided', async () => {
    mockFetch.mockResolvedValueOnce(errorResponse('ACCOUNT_LOCKED', 'Account is locked', 403));
...
describe('StorageService', () => {
  ...
  describe('file operations', () => {
    ...
    it('should GET /policy for getPolicy', async () => {
      mockFetch.mockResolvedValueOnce(jsonResponse({ data: { media_policy: 'safe_visible' } }));
...
describe('Service URL Paths', () => {
  const pathCases = [
    ...
    {
      name: 'queue enqueue',
      call: () => sm.queue.enqueue({ job_type: 't', payload: {} }),
      expectedPath: '/v1/queue/jobs'
    },
    {
      name: 'flagContent createFlag',
      call: () => sm.flagContent.createFlag({ content_type: 'a', content_id: 'b', category: 'c' }),
      expectedPath: '/v1/flagcontent/flags'
    }
  ];
...
describe('SchedulerService', () => {
  it('should POST /jobs for createJob', async () => {
    ...
    await sm.scheduler.createJob({
      name: 'daily-report',
      cron: '0 9 * * *',
      type: 'webhook',
      config: { url: 'https://x.com/report' }
    });
    const [url, init] = mockFetch.mock.calls[0];
...
describe('PermissionsService', () => {
  ...
  it('should POST /policies for createPolicy', async () => {
    ...
    await sm.permissions.createPolicy({
      name: 'allow-edit',
      effect: 'allow',
      actions: ['edit'],
      resources: ['posts/*']
...
describe('LoggerService', () => {
  ...
  it('queryLogs response shape should match LogQueryResponse', async () => {
    mockFetch.mockResolvedValueOnce(
      jsonResponse({
        data: {
          logs: [{ id: '1', service_name: 'auth', severity: 'info', message: 'ok', timestamp: '2026-01-01T00:00:00Z' }],
          total: 1,
          page: 1,
          limit: 20
        }
