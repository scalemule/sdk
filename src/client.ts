...
class RateLimitQueue {
  ...
  get isRateLimited(): boolean { return Date.now() < this.rateLimitedUntil }
}
...
class OfflineQueue {
  ...
  getAll(): OfflineQueueItem[] { return [...this.queue] }
  ...
  setOnlineCallback(cb: () => void): void { this.onOnlineCallback = cb }
  get length(): number { return this.queue.length }
  get online(): boolean {
...
export class ScaleMuleClient {
  ...
  setAccessToken(token: string): void { this.sessionToken = token }
  clearAccessToken(): void { this.sessionToken = null }
  ...
  getOfflineQueueLength(): number { return this.offlineQueue?.length || 0 }
  getRateLimitQueueLength(): number { return this.rateLimitQueue?.length || 0 }
  isRateLimited(): boolean { return this.rateLimitQueue?.isRateLimited || false }
  ...
  async upload<T>(
    ...
    const sanitizedFile = sanitizedName !== fileName
  ...
  private uploadWithXHR<T>(
    ...
    return new Promise(async (resolve) => {
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        // Check if retryable
        const isRetryable = result.error.code === 'upload_error' || result.error.code === 'network_error' ||
