...
export class LeaderboardService extends ServiceModule {
  ...
  async create(data: { name: string; sort_order?: 'asc' | 'desc' }, options?: RequestOptions): Promise<ApiResponse<Leaderboard>> {
    return this.post<Leaderboard>('', data, options)
  }

  async submitScore(boardId: string, data: { user_id: string; score: number }, options?: RequestOptions): Promise<ApiResponse<LeaderboardEntry>> {
    return this.post<LeaderboardEntry>(`/${boardId}/scores`, data, options)
  }

  async getRankings(boardId: string, rankingOptions?: { limit?: number; offset?: number }, requestOptions?: RequestOptions): Promise<ApiResponse<LeaderboardEntry[]>> {
  ...
  async getUserHistory(boardId: string, userId: string, options?: RequestOptions): Promise<ApiResponse<LeaderboardEntry[]>> {
    return this._get<LeaderboardEntry[]>(`/${boardId}/users/${userId}/history`, options)
  }

  async updateScore(boardId: string, userId: string, data: { score: number }, options?: RequestOptions): Promise<ApiResponse<LeaderboardEntry>> {
    return this.patch<LeaderboardEntry>(`/${boardId}/users/${userId}/score`, data, options)
  }

  async deleteScore(boardId: string, userId: string, options?: RequestOptions): Promise<ApiResponse<{ deleted: boolean }>> {
