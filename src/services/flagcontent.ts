...
export class FlagContentService extends ServiceModule {
  ...
  async checkFlag(params: { content_type: string; content_id: string }, requestOptions?: RequestOptions): Promise<ApiResponse<FlagCheck>> {
  ...
  async submitAppeal(data: { flag_id: string; reason: string }, options?: RequestOptions): Promise<ApiResponse<Appeal>> {
