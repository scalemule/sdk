...
export class PhotoService extends ServiceModule {
  ...
  async transform(photoId: string, transformations: Record<string, unknown>, options?: RequestOptions): Promise<ApiResponse<TransformResult>> {
  ...
  */
  getOptimalUrl(photoId: string, displayWidth: number, options?: { dpr?: number }): string {
    ...
    const size = PHOTO_BREAKPOINTS.find((bp) => bp >= physicalWidth)
      ?? PHOTO_BREAKPOINTS[PHOTO_BREAKPOINTS.length - 1]
  ...
  getSrcSet(photoId: string): string {
    return PHOTO_BREAKPOINTS
      .map((size) => `${this.getTransformUrl(photoId, { width: size, height: size, fit: 'cover' })} ${size}w`)
      .join(', ')
  }
   ...
   */
  async register(registerOptions: { fileId: string; userId?: string }, requestOptions?: RequestOptions): Promise<ApiResponse<PhotoInfo>> {
    return this.post<PhotoInfo>('/register', {
