...
class AuthMfaApi extends ServiceModule {
  ...
  async verifySetup(data: { code: string }): Promise<ApiResponse<{ success: boolean; backup_codes: string[]; message: string }>> {
  ...
  async disable(data: { password: string; code?: string }): Promise<ApiResponse<{ success: boolean; message: string }>> {
  ...
  async sendCode(data: { pending_token: string; method: 'sms' | 'email' }): Promise<ApiResponse<{ success: boolean; message: string; expires_in_seconds: number }>> {
    return this.post<{ success: boolean; message: string; expires_in_seconds: number }>('/send-code', data)
  }

  async verify(data: { pending_token: string; code: string; method?: 'totp' | 'sms' | 'email' | 'backup_code' }): Promise<ApiResponse<AuthSession>> {
...
export class AuthService extends ServiceModule {
  ...
  async login(data: { email: string; password: string; remember_me?: boolean }, options?: RequestOptions): Promise<ApiResponse<AuthSession>> {
  ...
  async refreshSession(data?: { refresh_token?: string; session_token?: string }, options?: RequestOptions): Promise<ApiResponse<AuthSession>> {
   ...
   */
  async signInWithOtp(data: { email?: string; phone?: string; channel?: 'email' | 'sms' }, options?: RequestOptions): Promise<ApiResponse<{ sent: boolean }>> {
    const payload = {
      ...data,
      phone: this.sanitizePhoneField(data.phone),
    }
   ...
   * @experimental Endpoint availability depends on backend deployment.
   */
  async verifyOtp(data: { email?: string; phone?: string; code: string }, options?: RequestOptions): Promise<ApiResponse<AuthSession>> {
    const payload = {
      ...data,
      phone: this.sanitizePhoneField(data.phone),
    }
   ...
   * @experimental Endpoint availability depends on backend deployment.
   */
  async signInWithMagicLink(data: { email: string }, options?: RequestOptions): Promise<ApiResponse<{ sent: boolean }>> {
  ...
  async sendPhoneOtp(data: { phone: string; purpose?: 'verify_phone' | 'login' | 'password_reset' | 'change_phone' }, options?: RequestOptions): Promise<ApiResponse<{ sent: boolean }>> {
    const payload = {
      ...data,
      phone: this.sanitizePhoneField(data.phone) ?? '',
    }
  ...
  async verifyPhoneOtp(data: { phone: string; code: string }, options?: RequestOptions): Promise<ApiResponse<{ verified: boolean }>> {
    const payload = {
      ...data,
      phone: this.sanitizePhoneField(data.phone) ?? '',
    }
  ...
  async loginWithPhone(data: { phone: string; code: string }, options?: RequestOptions): Promise<ApiResponse<AuthSession>> {
    const payload = {
      ...data,
      phone: this.sanitizePhoneField(data.phone) ?? '',
  ...
  async resetPassword(data: { token: string; password?: string; new_password?: string }, options?: RequestOptions): Promise<ApiResponse<{ reset: boolean }>> {
    return this.post<{ reset: boolean }>('/reset-password', data, options)
  }

  async changePassword(data: { current_password: string; new_password: string }, options?: RequestOptions): Promise<ApiResponse<{ changed: boolean }>> {
  ...
  async resendVerification(data?: { email?: string }, options?: RequestOptions): Promise<ApiResponse<{ sent: boolean }>> {
  ...
  async changeEmail(data: { new_email: string; password: string }, options?: RequestOptions): Promise<ApiResponse<{ changed: boolean }>> {
  ...
  async handleOAuthCallback(data: { provider: string; code: string; state?: string }, options?: RequestOptions): Promise<ApiResponse<AuthSession>> {
  ...
  async refreshAccessToken(data?: { refresh_token?: string }, options?: RequestOptions): Promise<ApiResponse<AuthSession>> {
    return this.post<AuthSession>('/token/refresh', data ?? {}, options)
  }

  async revokeRefreshToken(data: { refresh_token: string }, options?: RequestOptions): Promise<ApiResponse<{ revoked: boolean }>> {
  ...
  async revokeSession(sessionId: string) { return this.sessions.revoke(sessionId) }
  /** @deprecated Use auth.sessions.revokeAll() */
  async revokeOtherSessions() { return this.sessions.revokeAll() }

  /** @deprecated Use auth.devices.list() */
  async listDevices() { return this.devices.list() }
  /** @deprecated Use auth.devices.trust() */
  async trustDevice(deviceId: string) { return this.devices.trust(deviceId) }
  /** @deprecated Use auth.devices.block() */
  async blockDevice(deviceId: string) { return this.devices.block(deviceId) }
  /** @deprecated Use auth.devices.delete() */
  async deleteDevice(deviceId: string) { return this.devices.delete(deviceId) }
  ...
  async getLoginActivitySummary() { return this.loginHistory.getSummary() }

  /** @deprecated Use auth.mfa.getStatus() */
  async getMfaStatus() { return this.mfa.getStatus() }
  /** @deprecated Use auth.mfa.setupTotp() */
  async setupTotp() { return this.mfa.setupTotp() }
  /** @deprecated Use auth.mfa.verifySetup() */
  async verifyTotpSetup(data: { code: string }) { return this.mfa.verifySetup(data) }
  /** @deprecated Use auth.mfa.enableSms() */
  async enableSmsMfa() { return this.mfa.enableSms() }
  /** @deprecated Use auth.mfa.enableEmail() */
  async enableEmailMfa() { return this.mfa.enableEmail() }
  /** @deprecated Use auth.mfa.disable() */
  async disableMfa(data: { password: string; code?: string }) { return this.mfa.disable(data) }
  /** @deprecated Use auth.mfa.regenerateBackupCodes() */
  async regenerateBackupCodes() { return this.mfa.regenerateBackupCodes() }
  /** @deprecated Use auth.mfa.sendCode() */
  async sendMfaCode(data: { pending_token: string; method: 'sms' | 'email' }) { return this.mfa.sendCode(data) }
  /** @deprecated Use auth.mfa.verify() */
  async verifyMfa(data: { pending_token: string; code: string; method?: 'totp' | 'sms' | 'email' | 'backup_code' }) { return this.mfa.verify(data) }
}
