...
const COMPRESSIBLE_TYPES = new Set(['image/jpeg', 'image/jpg', 'image/png', 'image/bmp', 'image/tiff']);
const SKIP_TYPES = new Set(['image/gif', 'image/svg+xml', 'image/webp', 'image/avif']);
...
async function loadImageCompression(): Promise<ImageCompressionFn | null> {
  ...
  try {
    ...
    const mod = await (Function('return import("browser-image-compression")')() as Promise<{
      default?: ImageCompressionFn;
    }>);
