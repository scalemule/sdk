...
export class AgentSessionsService extends ServiceModule {
  ...
  async listSessions(
    params?: PaginationParams & { application_id?: string; agent_id?: string; status?: string; project_id?: string },
    options?: RequestOptions
  ...
  async endSession(
    id: string,
    data: { status: 'ended' | 'failed'; exit_code?: number; error_message?: string },
    options?: RequestOptions
  ...
  async appendLog(
    sessionId: string,
    data: { log_level: string; chunk_path: string; byte_size: number; line_count: number },
    options?: RequestOptions
  ...
  async addArtifact(
    ...
    options?: RequestOptions
  ): Promise<ApiResponse<SessionArtifact>> {
