...
export class LoggerService extends ServiceModule {
   ...
   */
  async log(data: LogInput | LegacyLogInput, options?: RequestOptions): Promise<ApiResponse<void>> {
   ...
   */
  async logBatch(logs: LogInput[], options?: RequestOptions): Promise<ApiResponse<{ ingested: number }>> {
    if (logs.length === 0) {
   ...
   */
  async queryLogs(filters?: LogQueryParams, requestOptions?: RequestOptions): Promise<ApiResponse<LogQueryResponse>> {
