...
export class ListingsService extends ServiceModule {
  ...
  async create(data: { title: string; description: string; price?: number; category?: string }, options?: RequestOptions): Promise<ApiResponse<Listing>> {
  ...
  async search(query: string, filters?: Record<string, unknown>, options?: RequestOptions): Promise<ApiResponse<Listing[]>> {
    return this._get<Listing[]>(this.withQuery('/search', { query, ...filters }), options)
  }

  async nearby(nearbyOptions: { lat: number; lng: number; radius: number; category?: string }, options?: RequestOptions): Promise<ApiResponse<Listing[]>> {
    return this._get<Listing[]>(this.withQuery('/nearby', nearbyOptions), options)
  }

  async getByCategory(category: string, params?: PaginationParams, requestOptions?: RequestOptions): Promise<PaginatedResponse<Listing>> {
