...
export class AgentAuthService extends ServiceModule {
  ...
  async registerAgent(
    data: AuthRegisterAgentRequest,
    options?: RequestOptions
  ): Promise<ApiResponse<AuthRegisterAgentResponse>> {
  ...
  async createToken(
    data: { name: string; scopes?: string[]; ip_allowlist?: string[]; expires_in_days?: number },
    options?: RequestOptions
  ): Promise<ApiResponse<{ id: string; token: string; name: string; expires_at?: string }>> {
  ...
  async rotateToken(
    id: string,
    options?: RequestOptions
  ): Promise<ApiResponse<{ new_token: string; old_token_grace_expires_at: string }>> {
    return this.post(`/agent-tokens/${id}/rotate`, undefined, options);
  }

  async exchangeToken(
    data: { refresh_secret: string; ttl_minutes?: number },
    options?: RequestOptions
  ...
  async addSigningKey(
    data: { public_key_pem: string },
    options?: RequestOptions
  ): Promise<ApiResponse<{ id: string; fingerprint: string }>> {
  ...
  async updateSecurityPolicy(
    appId: string,
    data: Partial<AgentSecurityPolicy>,
    options?: RequestOptions
  ): Promise<ApiResponse<AgentSecurityPolicy>> {
