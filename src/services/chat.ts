...
export class ChatService extends ServiceModule {
  ...
  async createConversation(data: { name?: string; participant_ids: string[] }, options?: RequestOptions): Promise<ApiResponse<Conversation>> {
    return this.post<Conversation>('/conversations', data, options)
  }

  async listConversations(params?: PaginationParams, requestOptions?: RequestOptions): Promise<PaginatedResponse<Conversation>> {
  ...
  async addParticipant(conversationId: string, userId: string, options?: RequestOptions): Promise<ApiResponse<{ added: boolean }>> {
    return this.post<{ added: boolean }>(`/conversations/${conversationId}/participants`, { user_id: userId }, options)
  }

  async removeParticipant(conversationId: string, userId: string, options?: RequestOptions): Promise<ApiResponse<{ removed: boolean }>> {
  ...
  async sendMessage(conversationId: string, data: { content: string; type?: string }, options?: RequestOptions): Promise<ApiResponse<ChatMessage>> {
    return this.post<ChatMessage>(`/conversations/${conversationId}/messages`, data, options)
  }

  async getMessages(conversationId: string, options?: { limit?: number; before?: string }, requestOptions?: RequestOptions): Promise<ApiResponse<ChatMessage[]>> {
    return this._get<ChatMessage[]>(this.withQuery(`/conversations/${conversationId}/messages`, options), requestOptions)
  }

  async editMessage(messageId: string, data: { content: string }, options?: RequestOptions): Promise<ApiResponse<ChatMessage>> {
  ...
  async addReaction(messageId: string, data: { emoji: string }, options?: RequestOptions): Promise<ApiResponse<ChatReaction>> {
